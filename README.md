# Assignment_1_id

Assignment 1 for ID: https://github.com/Cylinaho/Assignment_1_id.git
GITHUB PAGE: https://cylinaho.github.io/Assignment_1_id/

# BoyNextDoor Fan Website
## Project Overview:

The BoyNextDoor Fan Website is a simple, user-friendly fan hub dedicated to the K-POP boy group BOYNEXTDOOR. With the rising global popularity of K-POP, many fans want an easy and centralised way to learn more about their favourite artists without needing to jump between multiple apps or social media platforms. This project aims to provide a clean and accessible space for both new and long-time fans to explore information about the group in one place.

Essentially, this website acts as a digital home for fans—offering updated details about the members, recent news, band achievements, merchandise, and upcoming concert schedules. It is designed to be visually appealing, straightforward to use, and fully dedicated to enhancing the fan experience.

## Design Process:

This website is created for K-POP fans, especially those who support BOYNEXTDOOR or are curious to learn more about the group. Users want a platform where they can quickly find reliable information, stay updated on recent activities, and access merchandise or concert tickets without hassle.
The project helps users achieve these goals by placing all fan-related content in one central location, making navigation simple and intuitive.

## User Stories:

- As a new fan, I want to learn about each member so that I can familiarise myself with the group quickly.

- As a longtime fan, I want to stay updated with the latest BOYNEXTDOOR news so that I never miss important announcements.

- As a fan interested in merch, I want to browse and purchase official items so that I can support the group.

- As a concert-goer, I want to view upcoming events and book tickets easily so that I don’t miss performances.

- As a casual visitor, I want a simple navigation menu so that I can explore the site effortlessly.

## Wireframes & Mockups:

my figma link: https://www.figma.com/design/Evlfsuo3D01qPd3cS3md1k/BOYNEXTDOOR-HTML-DESIGN?node-id=0-1&t=rqfgZBu78VCflEYM-1

## Features
## Existing Features:

- Member Profiles – Allows users to learn about each BOYNEXTDOOR member by providing clear bios and photos.

- Achievements Timeline – Displays the group’s milestones and accomplishments over the years.

- Merchandise Store – Allows fans to browse and purchase official band merchandise in one place.

- Concert Booking Page – Lets users view upcoming tours and easily book tickets.

## Features Left to Implement:

- Fan Community Forum – A discussion space where fans can connect and share posts.

- Multilingual Support – Add Korean, Japanese, and Chinese language versions.

- Dark/Light Mode Toggle – For better visual comfort and customisation.

# Technologies Used:

- HTML – For structuring the web pages.

- CSS – For styling, layout, and responsive design.

- JavaScript – Used to add interactivity and dynamic functionality.

- Pinterest - For ideas of layout of a website.

- Figma – Used to design wireframes and plan the layout before development.

# Testing
## Manual Testing Scenarios:

Navigation Bar:

- Click through each link to confirm all pages load correctly.

- Test navigation on both desktop and mobile screens.

Index Page: 

- Test the layout is right for the images and text_box containers

- Test “CLICK HERE TO BUY” and "EXPLORE" buttons.

- Test Home icon links back to home page.

- Check if the privacy policy is link properly.

About Page:

- Check if the image is display right and the information are right.

- Test Home icon links back to home page.

- Check if the privacy policy is link properly

Product Page:

- Check if items load properly.

- Test “CLICK HERE TO BUY” buttons.

- Test Home icon links back to home page.

- Check if the privacy policy is link properly.

Member Page:

- Check if the layout for all the members are accurate.

- Test Home icon links back to home page.

- Check if the privacy policy is link properly.

Contact Page: 

- Check if the image is display right and the information is correct.

- Test Home icon links back to home page.

- Check if the privacy policy is link properly.

Concert Booking Page:

- Test ticket image layout is correct.

- Test the ticket type have a drop down list for users to interact.

- Check if the price matches with whatever user chooses.

- Test if the quantity work.

- Test Home icon links back to home page.

- Check if the privacy policy is link properly.

Product (selection and option) :

- Check if the layout are right.

- Test if i select first product it open a new page

- Test if the user can interact with the selection and the quantity.

- Test if the "ADD TO CART" button is working properly.

- Test Home icon links back to home page.

- Check if the privacy policy is link properly.

## Responsive Layout:

- Tested on Chrome

Notable Bugs Found:

- mobile doesnt work.

- Mobile menu layout may occasionally overlap when resizing the browser window.

# Credits
## Content:

- All BOYNEXTDOOR information written manually and referenced from verified sources such as official pages and interviews.

## Media:
- Pinterest for group photos and ID were used.

- Images used belong to KOZ Entertainment / HYBE (for educational project use).

- Icons sourced from free icon libraries such as Flaticon.

## Acknowledgements:

- Inspiration for this fan-site came from browsing multiple K-POP fan websites and wanting a simpler, cleaner alternative.

- Thanks to classmates and online tutorials for guidance throughout the design and development process.
